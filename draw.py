...
def draw(atoms, show_unit_cell=True,
    ...
    if isinstance(atoms, Atoms):
        ...
        print("Tried to draw invalid type:", type(atoms).__name__ + "."
              " Accepted types: str, ase.Atoms, pymatgen.Structure.")
        return

    vis = WrappedStructureVis(show_unit_cell=show_unit_cell,
                              show_bonds=show_bonds,
                              show_polyhedron=False,
     				 		  poly_radii_tol_factor=0.1)

    vis.set_structure(struct, reset_camera=True, to_unit_cell=not unwrap)
    vis.show()
